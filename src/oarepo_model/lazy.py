...
class LazyJSONNamespaceFilePart(LazyPythonMapping, abc.ABC):
    ...
    def __deepcopy__(self, memo: dict | None = None) -> Any:
        """Deep copy the mapping, without resolving it first."""
        return type(self)(
            model=self._model, keys=self._keys, filename=self._filename, initial_content=self._initial_content
        )
...
class LazyMarshmallowSchema(marshmallow.Schema):
    ...
    """Keys always resolved in addition to `keys`, with a fallback field to use if not found."""
    ...
    def proxied_schema(self) -> marshmallow.Schema:
        ...
        for key in keys:
            ...
            if field is None:
                field = self.extra_fields.get(key)
            if field is None:
