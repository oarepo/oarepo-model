...
def _descend_marshmallow_schema(schema: marshmallow.Schema, path: str) -> marshmallow.Schema:
    ...
    :raises ValueError: if a segment is missing or is not nested.
    """
    if not path:
        return schema
...
class ReferenceMappingProperties(LazyJSONNamespaceFilePart):
    ...
    def __deepcopy__(self, memo: dict | None = None) -> Any:
        """Deep copy the mapping, without resolving it first."""
        return type(self)(
            model=self._model,
            keys=self._keys,
    ...
    def _get_path(self, data: Any, path: str) -> dict[str, Any]:
        ...
        return walk_type_tree_path_leaf(data.get("mappings", {}).get("properties"), path)
    ...
    def _set_path(self, data: Any, path: str, value: Any) -> None:
        ...
        `data` is a full mapping node ("type"/"properties" wrapper and all),
        but `set_key_model` walks a bare "properties"-style dict (as used in
        relations.py) - one level has to be unwrapped here first, or it would
        set stray top-level keys on `data` instead of entries under
        `data["properties"]`.
        """
        data.setdefault("type", "object")
        set_key_model(data.setdefault("properties", {}), path, value, update=True)


#
# note: json schema properties are the same as mapping properties, but the path
# starts with "properties" instead of "mappings"
#
class ReferenceJSONSchemaProperties(ReferenceMappingProperties):
    ...
    def _get_path(self, data: Any, path: str) -> dict[str, Any]:
        ...
        if self._target_path:
            path = f"{self._target_path}.{path}"
        return walk_type_tree_path_leaf(data.get("properties"), path)
...
class ReferenceUIModel(LazyJSONNamespaceFilePart):
    ...
    def _get_path(self, data: Any, path: str) -> dict[str, Any]:
        ...
        Not every field has a UI model counterpart on the target (e.g. "id"
        is never synthesized there at all) - falls back to `{}`, finished off
        by `_set_path`'s own fallback model, rather than failing.
        """
        if self._target_path:
            path = f"{self._target_path}.{path}"
        try:
            return walk_ui_model_path_leaf(data.get("children", {}), path)
...
class ReferenceMarshmallowSchema(LazyMarshmallowSchema):
    ...
    # A relation is always stored keyed by "id" ({"id": ...}), regardless of
    # whether 'keys' lists it - see ReferenceMappingProperties/ReferenceUIModel's
    # own "id"/"@v" extra_fields above for the mapping/json schema/ui model
    # equivalent. "@v" is deliberately not included here: it mirrors
    ...
    def _get_target_schema(cls) -> marshmallow.Schema:
        ...
        if target_path:
            schema = _descend_marshmallow_schema(schema, target_path)
        return schema
...
class ReferenceUIMarshmallowSchema(LazyMarshmallowSchema):
    ...
    # Unlike the record schema, the UI schema isn't marked skip_marshmallow for
    # "@v" (create_ui_marshmallow_schema doesn't filter on it - see
    # ObjectDataType.create_ui_marshmallow_schema) - both "id" and "@v" are
    # always stored, so both must always be loadable/dumpable here too.
    extra_fields: Mapping[str, marshmallow.fields.Field] = {
        "id": marshmallow.fields.Raw(),
        "@v": marshmallow.fields.Raw(),
    }
    ...
    def _resolve_nested_relation_fields(
        ...
        for customization in self._build_nested_relation_customizations(element, path):
            ...
            for name, field in customization.build_relation_fields().items():
                # Only a nested AddLazyRelation yields a RelationsField *group*
                # (keyed by a synthetic name) instead of a single field; merge its
                # leaf fields in directly so the result stays a flat
                # {name -> field} mapping for the enclosing LazyRelationsField
                # (and, since we already run lazily, resolving its group is safe).
