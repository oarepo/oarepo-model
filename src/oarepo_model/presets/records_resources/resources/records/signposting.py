...
from oarepo_runtime.resources.signposting import record_to_json_linkset, record_to_linkset
...
class LinksetSignpostingSerializer(BaseSerializer):
    ...
    def serialize_object(self, obj: RecordItem) -> str | Any:
        """Serialize a single to a linkset. Record item is expected to have datacite export."""
        """Serialize a single object according to the response ctx."""
        return record_to_linkset(obj)
...
class JSONLinksetSignpostingSerializer(BaseSerializer):
    ...
    def serialize_object(self, obj: RecordItem) -> dict[str, list[dict[str, Any]]] | Any:
        """Serialize a single to a JSON linkset. Record item is expected to have datacite export."""
        return record_to_json_linkset(obj)
