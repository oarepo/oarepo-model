...
class TestFromJson:
    ...
    def _write_json(self, data: object, *, dest_dir: str) -> str:
        path = Path(dest_dir) / "types.json"
        path.write_text(json.dumps(data), encoding="utf-8")
        return str(path)

    def test_dict_format_returns_dict(self, tmp_path):
        path = self._write_json({"MyType": {"type": "keyword"}}, dest_dir=str(tmp_path))
        result = from_json(path)
        assert result == {"MyType": {"type": "keyword"}}

    def test_list_format_returns_dict_keyed_by_name(self, tmp_path):
        path = self._write_json([{"name": "MyType", "type": "keyword"}], dest_dir=str(tmp_path))
        result = from_json(path)
...
class TestGetTypeErrorPaths:
    ...
    def test_dict_without_type_or_properties_or_items_raises_value_error(self, datatype_registry):
        with pytest.raises(ValueError, match="Can not get type from"):
