...
def test_recursive_relations(
    ...
    # LazyPIDRelation in datatypes/lazy_relations.py) once this
...
def test_recursive_relations_mapping_and_jsonschema(app, recursive_relation_model):
    ...
    links = recursive_relation_model.__symlinks__
...
def test_recursive_relations_ui_model(app, recursive_relation_model):
    ...
    Like create_mapping/create_json_schema above (see ReferenceMappingProperties/
    ReferenceJSONSchemaProperties in datatypes/lazy_relations.py), PIDRelation.create_ui_model's
    'children' is backed by a lazily-resolved mapping (ReferenceUIModel) for
    self-referencing relations - it should reflect the real fields of the
    target model (e.g. "multilingual" resolving all the way down to its
    nested vocabulary "lang" field), instead of being left empty.
    """
